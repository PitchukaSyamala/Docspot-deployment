...
const Notification = () => {
   ...
   const handleAllMarkRead = async () => {
      try {
         const res = await axios.post('https://doctor-appointment-app-svx4.onrender.com/api/user/getallnotification', { userId: user._id }, {
            headers: {
   ...
   const handledeleteAllMark = async () => {
      try {
         const res = await axios.post('pitchuka-syamalas-projects/docspot-deployment/api/user/deleteallnotification', { userId: user._id }, {
            headers: {
               Authorization: `Bearer ${localStorage.getItem('token')}`,
            },
         })
